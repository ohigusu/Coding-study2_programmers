-- 코드를 작성해주세요
#테이블 GENERATION은 ID,PARENT_ID,GENERATION에 대한 정보가 담긴 테이블이다. GENERATION에 대한 정보를 얻기 위해 재귀 접근법 사용한다.
WITH RECURSIVE GENERATION AS
(
  SELECT ID, PARENT_ID, 1 AS GENERATION
  FROM ECOLI_DATA
  WHERE PARENT_ID IS NULL

  UNION ALL

  SELECT ed.ID, ed.PARENT_ID, g.GENERATION + 1 AS GENERATION
  FROM ECOLI_DATA AS ed
    INNER JOIN GENERATION AS g ON ed.PARENT_ID = g.ID
)

#자식이 없는 ID를 알기 위해 GENERATION 테이블을 기준으로 LEFT JOIN하여 PARENT_ID가 NULL인 행만 출력하여 GROUP BY,ORDER BY 진행한다.
SELECT
  COUNT(g.ID) AS 'COUNT',
  g.GENERATION AS 'GENERATION'
FROM GENERATION AS g
  LEFT JOIN ECOLI_DATA AS ed ON g.ID = ed.PARENT_ID
WHERE ed.PARENT_ID IS NULL
GROUP BY GENERATION
ORDER BY GENERATION;
